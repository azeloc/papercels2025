...
# Download data from the Sao Paulo State Court ----------------------------

baixados <- purrr::walk(base_analise$numeroProcesso, lex::tjsp_cpopg_api_download, path = "cpopg")

dados_parseados <- purrr::map_dfr(
  list.files("cpopg", full.names = TRUE),
  lex::tjsp_cpopg_api_parse
)

dados_parseados |>
  saveRDS("data/base_parseada.rds")
